...
SUM SPACE 3 ; Define an variable called SUM with size equal 3 byte.
SUM2 SPACE 3 ; Define a second variable called SUM2 with size equal 3 byte.
Result SPACE 10 ; Define a third variable called Result with size equal 10 byte.
...
Array DCB 31, 74, 46, 100, 10, 160, 123, 245, 16, 55 ; Define an array with size equal 10 byte.


Loop ; Loop to calculate the sum of numbers in the array;
	   LDRB R2, [R1] ; To bring one byte from array and stored it into a register R2.
	   ADD R3, R3, R2 ; Add the numbers in register R2 and R3 then the sum is in register R3.
...
Skip
	   ...
	   LDR R1, =SUM2 ; To bring the address of the SUM2 and stored it into a register R1.
	   STR R4, [R1] ; Stroe the value of R4 to SUM2.
...
Loop4 ; To find the number of zeros in the right.
	   ...
	   MOV R2, R2, LSR #1 ; Shift right register R2 by 1 bit.
